...
def scrape_site(site, dry_run=False):
    """
    ...
    with tqdm(desc=site['name'], unit=" pg", dynamic_ncols=True) as pbar:
        while True:
...
def fuzzy_search(query, df, limit=15, score_cutoff=80):
    """
    ...
    # Map each unique normalized form to one representative original title.
    # If two stores carry "Clank!" and "Clank", they collapse to one entry.
    norm_to_original = (
        df[['title', 'norm']]
...
def search_mode(query, sort_by='discount'):
    """
    Interactive search: fuzzy-match query against normalized titles in all
    scraped CSVs, prompt the user to pick one, then print the price table.
    """
    df = load_all_csvs()
...
def deals_mode(
    ...
    display.columns = [
        'Tienda', 'Producto', 'Precio', 'Oferta', 'Descuento', 'Disponibilidad', 'URL'
    ]
    ...
    for _, row in display.iterrows():
        wrapped_cells = {
            col: wrap_cell(row[col], col_widths[col])
            for col in display.columns
        }
...
def list_mode(store_filter=None, sort_by='store', in_stock_only=False):
    ...
    display = df[['store', 'title', 'original_price', 'current_price', 'descuento', 'stock_status']].copy()
    display.columns = ['Tienda', 'Producto', 'Precio', 'Oferta', 'Descuento', 'Disponibilidad']
...
"""
    ...
    if args.update:
        ...
        results = {}
        for site in targets:
            results[site['name']] = scrape_site(site, dry_run=args.dry_run)

        summary = pd.DataFrame([
            {
                'site':     name,
                'total':    len(df),
                'on_sale':  int(df['current_price'].notna().sum()) if not df.empty else 0,
                'agotado':  int((df['stock_status'] == 'Agotado').sum()) if not df.empty else 0,
            }
